...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const summary = await getService(id).catch(() => null);
...
export default async function ServiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const summary = await getService(id);
  if (!summary) notFound();
  ...
  const tabs = [
    {
      ...
      panel: <ServiceSuppliersPanel id={svc.id} />,
    },
    {
      key: 'apps',
      label: 'Applications',
      badge: summary.activeApps ?? undefined,
      panel: <ServiceApplicationsPanel id={svc.id} />,
    },
    { key: 'difficulty', label: 'Relay Mining', panel: <ServiceDifficultyPanel id={svc.id} /> },
    {
      key: 'raw',
      label: 'Raw',
